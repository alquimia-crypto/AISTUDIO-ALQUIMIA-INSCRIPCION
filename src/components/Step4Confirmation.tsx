...
export const Step4Confirmation: React.FC<Step4Props> = ({
  ...
  const whatsappMessage = encodeURIComponent(
    `¡Hola Academia Danza & Artes! He completado la inscripción de mi alumna ${student.Nombre_Alumna}.\n\n` +
      ...
      <div className="bg-gradient-to-b from-emerald-950 via-slate-900 to-slate-950 text-white rounded-3xl p-8 sm:p-10 shadow-2xl text-center relative overflow-hidden space-y-6">
        ...
        <div>
          ...
          <p className="text-emerald-100 text-sm mt-2 max-w-lg mx-auto">
            Hemos recibido los datos de la transferencia y el archivo se ha guardado de forma segura en Google Drive.
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 text-left border border-white/10 space-y-4 shadow-lg">
          ...
          {driveUrl && driveUrl !== 'Sin comprobante' && (
            <div className="pt-3 border-t border-white/10 flex items-center justify-between">
              <span className="text-xs text-slate-300 flex items-center space-x-1">
                <FileText className="w-4 h-4 text-emerald-400" />
                <span>Comprobante en Drive:</span>
              </span>
              <a
                href={driveUrl}
                target="_blank"
                rel="noreferrer"
                className="text-xs text-emerald-300 hover:text-emerald-200 underline font-medium inline-flex items-center space-x-1"
              >
        ...
        <div className="bg-slate-900/60 rounded-2xl p-5 text-left text-xs text-slate-300 space-y-2 border border-white/5">
          ...
          <ul className="space-y-1.5 pl-5 list-disc text-slate-300">
            ...
            <li>Una vez verificado el estado en Google Sheets (cambio a "Confirmado"), recibirás el correo definitivo con las indicaciones de inicio de clases.</li>
          </ul>
        </div>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
          <a
            href={`https://wa.me/593998765432?text=${whatsappMessage}`}
            target="_blank"
            rel="noreferrer"
